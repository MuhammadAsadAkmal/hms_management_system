...
def paramedic_signup_view(request):
    ...
    if request.method=='POST':
        ...
        return redirect(reverse('paramediclogin') + '?success_message=' + success_message)
...
def afterlogin_view(request):
    if is_admin(request.user):
        ...
        accountapproval=models.Paramedic.objects.all().filter(user_id=request.user.id,status=True)
        if accountapproval:
...
def admin_view_paramedic_view(request):
    ...
    return render(request,'hospital/admin_view_paramedic.html',{'paramedic':paramedic})
...
def admin_approve_paramedic_view(request):
    ...
    return render(request,'hospital/admin_approve_paramedic.html',{'paramedic':paramedic})
...
def update_paramedic_view(request,pk):
    ...
    if request.method=='POST':
        ...
        paramedicForm=forms.ParamedicForm(request.POST,request.FILES,instance=paramedic)
        if userForm.is_valid() and paramedicForm.is_valid():
            user=userForm.save()
            user.set_password(user.password)
            user.save()
...
def admin_view_facility_view(request):
    ...
    return render(request,'hospital/admin_view_facility.html',{'facilities':facilities})
...
def doctor_approve_appointment_view(request):
    ...
    appointments = models.Appointment.objects.filter(doctorId=doctor.get_id, status=False)
...
def doctor_discharge_patient_view(request):
    ...
    # return render(request,'hospital/doctor_view_patient.html',{'patients':patients,'doctor':doctor})
    patients=models.Patient.objects.all().filter(status=True,assignedDoctorId=request.user.id)
    return render(request,'hospital/doctor_discharge_patient.html',{'patients':patients})
...
def doc_discharge_patient_view(request,pk):
    ...
    return render(request,'hospital/doctor_patient_generate_bill.html',context=patientDict)
...
def paramedic_dashboard_view(request):
    ...
    'patientdischarged':patientdischarged,
    'appointments':appointments,
    'doctor':models.Paramedic.objects.get(user_id=request.user.id), #for profile picture of para in sidebar
    }
    mydict={
...
def paramedic_approve_patient_view(request):
    #those whose approval are needed
    patients=models.Patient.objects.all().filter(status=False)
    return render(request,'hospital/paramedic_approve_patient.html',{'patients':patients})
...
def paramedic_view_appointment_view(request):
    appointments=models.Appointment.objects.all().filter(status=True)
    return render(request,'hospital/paramedic_view_appointment.html',{'appointments':appointments})
...
def paramedic_approve_appointment_view(request):
    #those whose approval are needed
    appointments=models.Appointment.objects.all().filter(status=False)
    return render(request,'hospital/paramedic_approve_appointment.html',{'appointments':appointments})
...
@user_passes_test(is_paramedic)
def paramedic_appointment_view(request):
    paramedic=models.Paramedic.objects.get(user_id=request.user.id) #for profile picture of doctor in sidebar
...
def patient_view_review(request):
    pk = request.GET.get('pk')
    appointment=models.Appointment.objects.get(id=pk)
    return render(request,'hospital/patient_view_review.html',{'appointment':appointment})
...
def videocall(request):
    ...
    return render(request, 'hospital/videocall.html', {'name': request.user.first_name + " " + request.user.last_name, 'roomID': roomID})
